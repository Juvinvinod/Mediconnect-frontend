...
export class AddDoctorPopupComponent implements OnInit, OnDestroy {
  addDoctorSubscription: Subscription | undefined = undefined;
  departments: string[] = [
    'General surgery',
    'Cardiology',
    'Internal medicine'
  ...
  onSubmit() {
    if (this.doctorForm.valid) {
      this.addDoctorSubscription = this.doctorService
        .addDoctor(this.doctorForm.value)
        .subscribe({
          next: () => {
            this.ref.close();
